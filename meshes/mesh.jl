...
function readOFFacesFile(filePath)
    ...
    for line in startLine:(startLine+fCount-1)
        ...
        for p in 1:nPts
            ...
            # 0-based index
            # push!(facePts, parse(Int64, ptNumbers[p])+0)
        end
        push!(faces, facePts)
    end
...
function readOFOwnerFile(filePath)
    ...
    for line in startLine:(startLine+oCount-1)
        ...
        # 0-based index
        # faceOwnerCells[line-startLine+1] = parse(Int64, ownerLines[line])+0
    end
...
function readOFNeighbourFile(filePath)
    ...
    for line in startLine:(startLine+nCount-1)
        ...
        # 0-Based indexing
        # faceNeighbourCells[line-startLine+1] = parse(Int64, neighbourLines[line])+0
    end
